import{i as n}from"./darkmode-FyO9-oF3.js";import{h as o,n as d,f as m,a as p}from"./news-BNAMOQJL.js";import{a as t}from"./new-york-times-api-QNcKCIMv.js";import"./newsify_logo-BMn6THKq.js";const h=await t("world"),l=await t("health"),s=await t("arts"),r=await t("business"),c=await t("travel");function _(){const a=document.querySelector(".news__articles.europe"),e=h.filter(i=>i.subsection==="europe");Array.isArray(e)&&e.length>0?a.innerHTML=e.map(i=>`
        ...
        `}function u(){const a=document.querySelector(".news__articles.health");Array.isArray(l)&&l.length>0?a.innerHTML=l.map(e=>`
            <li class="news__article">
        ...
        `}function g(){const a=document.querySelector(".news__articles.arts");Array.isArray(s)&&s.length>0?a.innerHTML=s.map(e=>`
            <li class="news__article">
                <div class="news__div" data-article="${e.title}">
                    <img src="${e.multimedia?.[0]?.url||"https://placecats.com/g/100/100"}" alt="${e.title}">
                    <h3>${e.title}</h3>
        ...
        `}function w(){const a=document.querySelector(".news__articles.business");Array.isArray(r)&&r.length>0?a.innerHTML=r.map(e=>`
            <li class="news__article">
                <div class="news__div" data-article="${e.title}">
                    <img src="${e.multimedia?.[0]?.url||"https://placecats.com/g/100/100"}" alt="${e.title}">
                    <h3>${e.title}</h3>
        ...
        `}function v(){const a=document.querySelector(".news__articles.travel");Array.isArray(c)&&c.length>0?a.innerHTML=c.map(e=>`
            <li class="news__article">
                <div class="news__div" data-article="${e.title}">
                    <img src="${e.multimedia?.[0]?.url||"https://placecats.com/g/100/100"}" alt="${e.title}">
                    <h3>${e.title}</h3>
    ...
    <main></main>
    <footer></footer>
    `;document.querySelector("header").append(o());document.querySelector("main").append(d());document.querySelector("footer").append(m());n();p();window.addEventListener("DOMContentLoaded",()=>{_(),u(),g(),w(),v()});
